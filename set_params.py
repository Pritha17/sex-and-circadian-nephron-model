...
def read_params(cell,filename,j):
    ...
    # parameter files specify superficial segmental lengths; for some segments, lengths are different for juxtamedullary nephrons, so we overwrite them here...
            ...
            # Specific transporters:
                ...
                # circadian regulated transporters
                if newTransp.type == 'NHE3':
                    NHE3_0 = newTransp.act
                    zt = 14     # Change the zt value accordingly
                    theta_NHE3 = 7  # Data
                    gamma_NHE3 = 0.2    # Male: 0.1; Female: 0.2
                    newTransp.act = (1 + gamma_NHE3 * math.sin((2 * math.pi * (zt + 6 - theta_NHE3)) / 24)) * NHE3_0

                if newTransp.type == 'SGLT1':
                    SGLT1_0 = newTransp.act
                    zt = 14
                    theta_SGLT1 = 9  # Assumption; based on CircaDB data; SGLT1 mRNA expression peaks 2h after NHE3 mRNA expression peaks in mice
                    gamma_SGLT1 = 0.2
                    newTransp.act = (1 + gamma_SGLT1 * math.sin((2 * math.pi * (zt + 6 - theta_SGLT1)) / 24)) * SGLT1_0

                if newTransp.type == 'NKCC2A' or newTransp.type == 'NKCC2B' or newTransp.type == 'NKCC2F':
                    NKCC2_0 = newTransp.act
                    zt = 14
                    theta_NKCC2 = 16
                    gamma_NKCC2 = 0.8   # Male: 0.9; Female: 0.8
                    newTransp.act = (1 + gamma_NKCC2 * math.sin((2 * math.pi * (zt + 6 - theta_NKCC2)) / 24)) * NKCC2_0

                if newTransp.type == 'NCC':
                    NCC_0 = newTransp.act
                    zt = 14
                    theta_NCC = 16  # Data
                    gamma_NCC = 0.8 # Male: 0.9; Female: 0.8
                    newTransp.act = (1 + gamma_NCC * math.sin((2 * math.pi * (zt + 6 - theta_NCC)) / 24)) * NCC_0

                if newTransp.type == 'ENaC':
                    ENaC_0 = newTransp.act
                    zt = 14
                    theta_ENaC = 4  # Data
                    gamma_ENaC = 0.1
                    newTransp.act = (1 + gamma_ENaC * math.sin((2 * math.pi * (zt + 6 - theta_ENaC)) / 24)) * ENaC_0
            ...
            # Actual volume flows:
                ...
                theta_vol = 14
                zt = 14
                cell.vol[0] = (1 + gamma_vol * math.sin((2 * math.pi * (zt + 6 - theta_vol)) / 24)) * vol_0
                cell.vol_init[0] = cell.vol[0]
